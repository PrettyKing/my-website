...
而对于 classic 模式，其解析逻辑可能不太符合直觉，其相对路径导入与绝对路径导入均不会解析 `node_modules` 中的文件。对于相对路径导入 `import foo from "./foo"`，它只会尝试 `/<root>/<project>/src/index.ts` 和 `/<root>/<project>/src/index.d.ts`。
